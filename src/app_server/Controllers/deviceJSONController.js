...
var DEVICE_MEMORY_DIR = path.join(__dirname, '..', '..', 'config', 'deviceMemory');
...
function readLparData(lpar, callback) {
    var filePath = path.join(DEVICE_MEMORY_DIR, lpar + '.json');
    ensureDirectoryExists(DEVICE_MEMORY_DIR, function(err) {
        if (err) {
            return callback(err);
        }
